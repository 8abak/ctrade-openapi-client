...
def pelt(prefix: PrefixSums, penalty: float, min_len: int = 2) -> List[int]:
    ...
    # Standard trick: make first segment pay penalty once
    F[0] = -penalty
    ...
    for t_excl in range(1, n + 1):
        ...
        new_R: List[int] = []
        for s in R:
            # If [s, t_excl) is shorter than min_len, keep s unconditionally.
            if t_excl - s < min_len:
                new_R.append(s)
                continue
...
def refine_segment(
    ...
    with conn, dict_cur(conn) as cur:
        ...
        # --- ticks inside parent ---
        # NOTE: live DB uses ticks.timestamp
        cur.execute(
            """
        ...
        for _it in range(max_iters):
            ...
            if best_ends is None:
                ...
                # Prefer K in [target_min_segs, target_max_segs] and closest to target_min_segs (smallest K in range).
                # If none reached target_min_segs yet, prefer largest K achieved.
                better = False
        ...
        print(
            f"[refineL2] parent={parent_segment_id} symbol={symbol} "
            f"children={total_children} best_c2={best_c2:.6g} K={best_K} run_id={run_id}"
        )

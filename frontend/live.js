...
function redraw() {
  ...
  // Apply FULL option every time to guarantee axis models exist
  chart.setOption({
  ...
  if (followTail) {
    // Don’t assume existing dataZoom state; dispatch to tail explicitly
    chart.dispatchAction({ type: 'dataZoom', end: 100 });
  }
}

...
// - Exact float plotting; y-axis labels/grid are integers only
// - Auto y min/max (data window)
// - Collapsible journal (persists)
// - Tooltip: id, date, time, actual Mid/Bid/Ask + any Events/Preds/Outcomes at that tick
// - Gestures: drag L/R = zoom X, drag U/D = zoom Y; wheel zoom too
...
(function () {
  ...
  let prByTick = new Map();    // tick_id -> [preds joined to event]
  let ocByTick = new Map();    // tick_id -> [outcomes joined to event]
  let chart;
  ...
  function buildOption(ticks, snap, startId, endId) {
    ...
    // price series (exact floats for geometry)
    ...
    const base = {
      ...
      tooltip: {
        ...
        formatter: (params) => {
          ...
          if (evs?.length) {
            for (const e of evs) lines.push(`• <b>Event</b> ${e.event_type}`);
          }
          const prs = prByTick.get(tickId);
          if (prs?.length) {
            for (const p of prs) lines.push(`• <b>Pred</b> p_tp=${(p.p_tp ?? 0).toFixed(3)} τ=${(p.threshold ?? 0).toFixed(3)} [${p.model_version || '-'}]`);
          }
          const ocs = ocByTick.get(tickId);
          if (ocs?.length) {
            for (const o of ocs) lines.push(`• <b>Outcome</b> ${o.outcome}`);
          }
      ...
      yAxis: {
        ...
        min: 'dataMin',
        max: 'dataMax',
        minInterval: 1,                             // grid/labels at integers
      ...
      dataZoom: [
        ...
        { type: 'inside', xAxisIndex: 0, filterMode: 'none',
          ...
          zoomOnMouseMove: true,  zoomOnMouseWheel: true, throttle: 30 },
        // Y zoom: drag & wheel
    ...
    if ((snap?.predictions || []).length && evsWin.length) {
      const latest = snap.predictions;  // already latest per event in backend
      const rows = [];
    ...
    // initial legend states from toggles
    const sel = {};

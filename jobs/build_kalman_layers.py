...
def kalman_pass(y: np.ndarray, q: float, r: float, deadband: float = 0.0,
                x0: Optional[float] = None, P0: Optional[float] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    y = np.asarray(y, float)
    n = len(y)
    x_f = np.empty(n); P_f = np.empty(n)
    x_p = np.empty(n); P_p = np.empty(n)
    x = y[0] if x0 is None else float(x0)
    P = (r if P0 is None else float(P0))
...
# -------- Constant-velocity Kalman on a series --------
def kalman_cv_pass(z: np.ndarray, q_pos: float, q_vel: float, r: float,
                   x0: Optional[np.ndarray] = None, P0: Optional[np.ndarray] = None, dt: float = 1.0) -> np.ndarray:
    ...
    R = np.array([[r]])
    x = np.array([z[0], 0.0], dtype=float) if x0 is None else x0.astype(float)
...
# ---- NEW: CV + deadband (old behaviour) ----
K1_Q_SCALE      = float(os.getenv("K1_Q_SCALE",      "1e-3"))
K1_R_SCALE      = float(os.getenv("K1_R_SCALE",      "1.0"))
K1_DEADBAND_SIG = float(os.getenv("K1_DEADBAND_SIG", "0.50"))  # tighten if too flat
...
def kalman_cv_deadband(y: np.ndarray, q_scale: float, r_scale: float, deadband_sig: float,
    ...
    r     = (mad_dy ** 2) * r_scale  # measurement noise from dy
    Q = np.diag([q_pos, q_vel])
    R = np.array([[r]])
    ...
    for i in range(n):
        ...
        # gate: skip small changes -> straight segments with sharp turns
...
def main() -> None:
    ...
    # robust q/r from window
    q1, r1 = robust_qr_from(mids)

    # k1: OLD CV+deadband Kalman (what you liked)
    x0_k1 = None if last_k1 is None else np.array([last_k1, 0.0], dtype=float)
    k1 = kalman_cv_deadband(mids, q_scale=K1_Q_SCALE, r_scale=K1_R_SCALE, deadband_sig=K1_DEADBAND_SIG, x0=x0_k1, P0=None, dt=1.0)

    # k1_rts: keep your scalar + RTS behaviour exactly
    k1_scalar, P_f, x_p, P_p = kalman_pass(mids, q=q1, r=r1, deadband=0.0, x0=last_k1, P0=r1)
    k1_rts = rts_smoother(k1_scalar, P_f, x_p, P_p)

    # k2_cv: leave as your existing CV pass on k1_rts (or on k1); here keep your previous: CV on k1
    qpos = max(q1, 1e-9); qvel = max(0.01 * qpos, 1e-10)
    x0_cv = None if (last_tickid is None or last_k2 is None) else np.array([last_k2, 0.0], dtype=float)
    k2_cv = kalman_cv_pass(k1, q_pos=qpos, q_vel=qvel, r=r1, x0=x0_cv, P0=None)

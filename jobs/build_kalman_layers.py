...
def main() -> None:
    ...
    # Noise estimates from current window
    q1, r1 = robust_qr_from(mids)

    # Pass 1: scalar Kalman (seed from previous if available)
    x0_k1: Optional[float] = last_k1 if last_k1 is not None else None
    ...
    x0_cv: Optional[np.ndarray] = None
    if (last_tickid is not None) and (last_k2 is not None):
        x0_cv = np.array([last_k2, 0.0], dtype=float)
